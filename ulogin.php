...
if(iADMIN) {
	...
	function selectToUlogin(){
		global $settings2,$locale;
		$ug = dbquery( "SELECT group_id,group_name FROM " . DB_PREFIX . "user_groups " );
		if (dbrows($ug)){
			echo "<td width='50%' class='tbl'>".$locale['5551']."</td>\n";
			...
			echo "<option value=''".$sel_no.">".$locale['519']."</option>\n";
			echo "</select></td>\n";
			echo "</tr>\n<tr>\n";
		}
	}
...
/* если гость - готовим uLogin к работе */
	...
	function register_user($id,$pass) {	
		...
		/* Если есть имя, логин и пароль - проводим аутентификацию и возвращаемся  на главную */
	...
	if(isset($_POST['token'])){
		...
		if(dbrows($member_b))
			...
			//For Non-Unicode version of PHP-Fusion -------------------------------------------------
                        //    convert_unicode($city);								//
                        //    convert_unicode($login);								//
			...
			if ($alg == 'sha256') {
				$password = hash_hmac('sha256', $user['identity'], $seed);
			} else {
				$password = md5(md5($user['identity']));
			}
			...
			dbquery( "INSERT INTO " . DB_PREFIX . "users (user_name, user_salt, user_password, user_email, user_joined, user_lastvisit, user_rights, user_groups, user_level, user_threads, user_ip, user_sig, user_birthdate, user_location)
										VALUES ('$login', '$seed', '$password', '$email', '$time', '$time', '', '" . $settings['reg_group_ulogin'] . "', '101', '', '" . $_IP . "', '', '$user_birthdate', '$user_location')" );
			...
			if (dbrows($u_id)){
				...
				register_user($user_id,$user['identity']);
			}
		}
		unset($_POST['token']);
	}
